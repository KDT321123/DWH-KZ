...
describe("index-create-destination", () => {
    ...
    beforeEach(() => {
        bas_sdk_sinon_helper_1.stubEnv({ H2O_URL: H2O_URL, WS_BASE_URL: WS_BASE_URL });
    });
    afterEach(() => {
        mockAxios.reset();
    ...
    describe("createDestination", () => {
        it("create new destination", async () => {
            mockDestReply(200);
            await chai_1.expect(createDestination(expectedDestinationInfo)).to.be.fulfilled;
        });
        it("create new destination fail because there is already a destination with the same name (409)", async () => {
            mockDestReply(409);
            await chai_1.expect(createDestination(expectedDestinationInfo)).to.be.rejectedWith("Couldn't create the destination from the (https://h2o.cfapps.test.hana.ondemand.com/api/createDestination) URL because of the following error: Request failed with status code 409. Error code=409");
        });
        it("create new destination fail because body data is empty or other server issue", async () => {
            mockDestReply(500);
            await chai_1.expect(createDestination(expectedDestinationInfo)).to.be.rejectedWith("Couldn't create the destination from the (https://h2o.cfapps.test.hana.ondemand.com/api/createDestination) URL because of the following error: Request failed with status code 500. Error code=500");
        });
        it("throw exception when H2O_URL is not defined", async () => {
            bas_sdk_sinon_helper_1.stubEnv({ H2O_URL: undefined });
            await chai_1.expect(createDestination(expectedDestinationInfo)).to.be.rejectedWith("The H2O_URL environment variable is missing.");
        });
    });
});
