...
describe("index-core-runCommand", () => {
    describe("runCommand", () => {
        it("return the string of the command executed", async () => {
            ...
            chai_1.expect(isVersion).to.be.true;
        });
        it("throw error when command does not exist", async () => {
            await chai_1.expect(runCommand("cmd_fake", [])).to.be.rejectedWith("spawn cmd_fake ENOENT");
        });
        it("throw error when command fails", async () => {
            await chai_1.expect(runCommand("node", ["--a"]) // There is no parameter a for node
            ).to.be.rejectedWith();
        });
    });
});
