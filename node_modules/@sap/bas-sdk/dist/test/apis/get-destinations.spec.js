...
describe("index-get-destinations", () => {
    ...
    beforeEach(() => {
        bas_sdk_sinon_helper_1.stubEnv({ H2O_URL: H2O_URL, HRTT_URL: "", WS_BASE_URL: WS_BASE_URL });
    });
    afterEach(() => {
        mockAxios.reset();
    ...
    describe("getDestinations", () => {
        ...
        it("return http destinations", async () => {
            ...
            chai_1.expect(await getDestinations()).to.deep.equal(expectedDestinations);
        });
        it("return an empty destinations when the payload is empty", async () => {
            mockDestReply(200, []);
            chai_1.expect(await getDestinations()).to.deep.equal([]);
        });
        ...
        it("return empty DestinationListInfo array when one of the filter property does not exist in responseDestination", async () => {
            ...
            chai_1.expect(await getDestinations(filter)).to.deep.equal([]);
        });
        ...
        it("return only the responseDestinations that match the filter", async () => {
            ...
            chai_1.expect(await getDestinations(filter)).to.deep.equal(expectedDestinations);
        ...
        it("throw exception when H2O_URL is not defined", async () => {
            ...
            await chai_1.expect(getDestinations()).to.be.rejectedWith("The H2O_URL environment variable is missing.");
        });
        it("fails to retrieve destinations when HTTP request is failed", async () => {
            mockDestReply(500, "failed to retrieve destination");
            await chai_1.expect(getDestinations()).to.be.rejectedWith("Couldn't retrieve the destination from the (https://h2o.cfapps.test.hana.ondemand.com/api/listDestinations) URL because of the following error: Request failed with status code 500. Error code=500");
        });
        it(`fails to retrieve destinations when the payload is not a JSON array`, async () => {
            mockDestReply(200, `I'M NOT JSON array`);
            await chai_1.expect(getDestinations()).to.be.rejectedWith("The response data (I'M NOT JSON array) is not an array format");
        });
    });
});
