...
describe("index", () => {
    ...
    describe("HrttHelper", () => {
        ...
        beforeEach(() => {
            ...
            chai_1.expect(hrttHelper).to.be.not.undefined;
        });
        describe("getUrl", () => {
            ...
            it(`return the "HRTT_URL" from the "host" field within hrtt-host destinations`, async () => {
                sinon_1.stub(dest, "getDestinations").returns(Promise.resolve(expectedDestinations));
                chai_1.expect(await hrttHelper.getUrl(EXPECTED_HRTT_URL_FROM_DEST)).to.equal(EXPECTED_HRTT_URL_FROM_DEST);
            });
            ...
            it(`returns the "HRTT_URL" environment variable when there is no "hrtt-host" destination`, async () => {
                bas_sdk_sinon_helper_1.stubEnv({ HRTT_URL: EXPECTED_HRTT_URL_FROM_ENV });
                sinon_1.stub(dest, "getDestinations").returns(Promise.resolve([]));
                chai_1.expect(await hrttHelper.getUrl(CF_API_ENDPOINT_URL)).to.equal(EXPECTED_HRTT_URL_FROM_ENV);
            });
            it(`returns the "HRTT_URL" environment variable when destinations cannot be fetched`, async () => {
                bas_sdk_sinon_helper_1.stubEnv({ HRTT_URL: EXPECTED_HRTT_URL_FROM_ENV });
                sinon_1.stub(dest, "getDestinations").returns(Promise.resolve(undefined));
                chai_1.expect(await hrttHelper.getUrl(CF_API_ENDPOINT_URL)).to.equal(EXPECTED_HRTT_URL_FROM_ENV);
            });
            it(`returns the "HRTT_URL" environment variable when destinations return undefined`, async () => {
                bas_sdk_sinon_helper_1.stubEnv({ HRTT_URL: EXPECTED_HRTT_URL_FROM_ENV });
                sinon_1.stub(dest, "getDestinations").throwsException("failed to retrieve destination");
                chai_1.expect(await hrttHelper.getUrl(CF_API_ENDPOINT_URL)).to.equal(EXPECTED_HRTT_URL_FROM_ENV);
            });
            it(`returns the URL based on the endpoint parameter when there are no destinations or HRTT_URL env var`, async () => {
                sinon_1.stub(dest, "getDestinations").throwsException("failed to retrieve destination");
                chai_1.expect(await hrttHelper.getUrl(CF_API_ENDPOINT_URL)).to.equal(EXPECTED_HRTT_URL_FROM_CF);
            });
            it("fails to get HRTT URL when there are no destinations and environment is not configured and CF enpoint is missing", async () => {
                bas_sdk_sinon_helper_1.stubEnv({ HRTT_URL: "" });
                sinon_1.stub(dest, "getDestinations").returns(Promise.resolve([]));
                await chai_1.expect(hrttHelper.getUrl("")).to.be.rejectedWith("endpoint is missing");
            });
        });
    });
});
    ...
    describe("HrttHelper", () => {
        ...
        describe("getUrlFromCFendpoint-checkUrl", () => {
            ...
            hrttUrlEndpointUseCases.forEach(function (useCase) {
                it(useCase.desc, async function () {
                    chai_1.expect(await hrttHelper.getUrl(useCase.cfUrl)).to.equal(useCase.expectedHrrtUrl);
                });
            });
        });
        ...
        describe("getUrlFromCFendpoint - check invalid url that throw exception", () => {
            ...
            hrttUrlEndpointUseCases.forEach(function (useCase) {
                it(useCase.desc, async function () {
                    hrttHelper = createHrttHelper();
                    await chai_1.expect(hrttHelper.getUrl(useCase.cfUrl)).to.be.rejectedWith(useCase.expMsg);
                });
            });
        });
    });
