...
function isAllowtoAddDestination(responseDestination, filter) {
    return (lodash_1.isPlainObject(responseDestination) &&
        !lodash_1.isUndefined(getDestinationType(responseDestination.Type)) &&
        !lodash_1.isUndefined(getAuthenticationType(responseDestination.Authentication)) &&
...
function transformToDestinations(responseDestinations, filter) {
    ...
    responseDestinations.forEach((responseDestination) => {
        if (isAllowtoAddDestination(responseDestination, filter)) {
            destinations.push(destinations_1.responseDestinationToDestinationListInfo(responseDestination));
        }
    });
    return destinations;
}
...
async function getDestinations(filter) {
    const reqURL = url_join_1.default(core_utils_1.getEnvValue(destinations_1.ENV_H2O_URL), "/api/listDestinations");
    let resp;
    try {
        const reqConfig = {
            url: reqURL,
            headers: destinations_1.getDestinationHeaders(),
        };
